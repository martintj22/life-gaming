...
/* System Settings */
    ...
    escapeMenu_displayText = "Life-gaming - Custom Altis Life!"; //Text displayed in the escape menu. Make it short.. around 20 characters.

    /* Fuel System Configurations */
    pump_service = true; //Allow users to use pump service on the map. Default = false
    fuel_cost = 900; //Cost of fuel per liter at fuel stations (if not defined for the vehicle already).
    service_chopper = 3000; //Cost to service chopper at chopper service station(Repair/Refuel).
    fuelCan_refuel = 1400; //Cost to refuel an empty fuel canister at the fuel station pumps. (Be wary of your buy/sell prices on fuel cans to prevent exploits...)
    ...
    revive_fee = 3000; //Revive fee that players have to pay and medics only EMS(independent) are rewarded with this amount.
    ...
    seize_uniform[] = { "U_Rangemaster,","U_B_FullGhillie_ard","U_B_FullGhillie_sard" }; //Any specific uniforms you want to be seized from players
    seize_vest[] = { "V_TacVest_blk_POLICE","V_Press_F" }; //Any specific vests you want to be seized from players
    ...
    vehicle_rental_multiplier_CIVILIAN = .40; //Civilian Vehicle Rental Price = Config_Vehicle price * multiplier
    ...
    vehicle_chopShop_multiplier = .30; //Chop Shop price for vehicles. TO AVOID EXPLOITS NEVER SET HIGHER THAN A PURCHASE/RENTAL multipler!   Payout = Config_vehicle Price * multiplier
